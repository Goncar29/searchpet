...
func (m *mockPetRepo) FindStrayCandidates(_ domain.StrayCandidateCriteria) ([]domain.StrayCandidate, error) {
	return nil, nil
}
...
func TestCreatePet_StrayHasNilOwnerAndReporter(t *testing.T) {
	...
	svc := service.NewPetService(repo, nil, nil, reportRepo, uow, nil, nil, nil)
...
func TestCreatePet_StrayPublishesPetStrayEvent(t *testing.T) {
	...
	reportRepo := &mockReportRepo{}
	uow := &mockUnitOfWork{repos: repository.UnitOfWorkRepos{Pets: repo, Reports: reportRepo}}
	svc := service.NewPetService(repo, bus, nil, reportRepo, uow, nil, nil, nil)
...
func TestCreatePet_RejectsInvalidCreationStatuses(t *testing.T) {
	ownerID := uuid.New()
	svc := service.NewPetService(&capturingPetRepo{}, nil, nil, nil, nil, nil, nil, nil)
...
func TestCreatePet_Stray_RequiresInitialReport(t *testing.T) {
	repo := &mockPetRepo{}
	bus := event.NewEventBus()
	svc := service.NewPetService(repo, bus, nil, &mockReportRepo{}, nil, nil, nil, nil)
...
func TestCreatePet_Stray_WithInitialReport_CreatesPetAndReport(t *testing.T) {
	...
	bus := event.NewEventBus()
	svc := service.NewPetService(petRepo, bus, nil, reportRepo, uow, nil, nil, nil)
